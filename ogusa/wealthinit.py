...
def MVKDE(
    S, s_min, J, proportion_matrix, bandwidth=0.25, plot=False, plot_title=None
):
    """
    Generates a Multivariate Kernel Density Estimator (MVKDE) and returns a
    matrix representing a probability distribution according to given age
    categories and lifetime income group categories. This functions uses the
    scipy.stats.kde kernel density estimator package and calls the
    gaussian_kde() function.

    Args:
        S (int >= 3): Number of age groups in the model.
        s_min (int): Minimum or starting age in the age distribution of S
        J (int >= 1): Number of lifetime income groups in the model.
        proportion_matrix (NumpyArray S x J): Array containing the proportion
            matrix of the 2D joint distribution (e.g., wealth dist or bequests
            dist).
        plot (bool): Plot the KDE smoothed distributions if =True.

    Objects in function:
        proportion_matrix (array S x J): Array containing the proportion
            (0 < x < 1) of the total wealth or bequests that each age-income
            category receives.
        s_probs (vector len-S): Vector containing the frequency, or how many
            times, that random drawn numbers fell into the S different age bins
        j_probs (vector len-J): Vector containing the frequency, or how many
            times, that random drawn numbers fell into the J different lifetime
            income group bins
        s_frequency (vector len-70_000): Vector containing repeated age values
            for each age s at the frequency given by the s_probs vector
        j_frequency (vector len-70_000): Vector containing repeated lifetime
            income group values for each j number at the frequency given by the
            j_probs vector
    ...
    Returns:
        estimator_scaled (array S x J): Smoothed and scaled 2 dimensional
            distribution
    """
    dist_j = np.sum(proportion_matrix, axis=0)
    dist_s = np.sum(proportion_matrix, axis=1)
    s_probs = np.random.multinomial(70000, dist_s)
    j_probs = np.random.multinomial(70000, dist_j)
    s_frequency = np.array([])
    j_frequency = np.array([])
    s_mesh = complex(str(S) + "j")
    j_mesh = complex(str(J) + "j")
    ...
    for i in s_probs:
        listit = np.ones(i)
        listit *= s
        s_frequency = np.append(s_frequency, listit)
        s += 1
    ...
    for i in j_probs:
        listit2 = np.ones(i)
        listit2 *= j
        j_frequency = np.append(j_frequency, listit2)
        j += 1

    freq_mat = np.vstack((s_frequency, j_frequency)).T
    ...
    agei, incomei = np.mgrid[
        age_min:age_max:s_mesh, income_min:income_max:j_mesh
    ]
    ...
    if plot:
        ...
        ax.set_zlabel("Percent of total")
